...
from preprocessing import load_and_clean_auxiliary_data, merge_auxiliary_data, extract_time_features

warnings.simplefilter(action='ignore', category=FutureWarning)


def create_sales_timeline_fig(df):
    """Create daily sales timeline with global and top 5 stores"""
...
def create_family_sales_fig(df):
    """Create sales timeline for top 12 product families"""
...
def create_monthly_box_fig(df):
    """Create box plot of monthly sales distribution"""
    monthly = df.copy()
    ...
    fig = px.box(month_agg, x='month', y='sales', points='outliers', 
                 title='Distribution of Monthly Sales by Month', height=450)
...
def create_weekday_analysis_fig(df):
    ...
    fig.update_layout(title="Average Sales by Weekday", xaxis_title="Weekday", 
                      yaxis_title="Average Sales", height=450)
    return fig


def create_oil_correlation_figs(df):
    """Create oil price correlation visualizations"""
    oil_daily = df[['date', 'dcoilwtico', 'sales']].dropna().groupby('date', as_index=False).agg({
        'dcoilwtico': 'mean', 'sales': 'sum'
    })
    
    oil_scatter_fig = px.scatter(oil_daily, x='dcoilwtico', y='sales', trendline='ols', 
                                 title='Oil Price vs Sales (Scatter)', height=450)
    
    oil_timeseries_fig = go.Figure()
    oil_timeseries_fig.add_trace(go.Scatter(x=oil_daily['date'], y=oil_daily['sales'], name='Sales'))
    oil_timeseries_fig.add_trace(go.Scatter(x=oil_daily['date'], y=oil_daily['dcoilwtico'], 
                                             name='Oil Price', yaxis="y2"))
    oil_timeseries_fig.update_layout(
        title='Sales & Oil Price Over Time',
...
def create_geographic_sales_figs(df):
    """Create geographic sales visualizations"""
    city_sum = df.groupby('city', as_index=False)['sales'].sum().sort_values('sales', ascending=False)
    city_treemap_fig = px.treemap(city_sum, path=[px.Constant("All Cities"), 'city'], 
                                   values='sales', title='Sales Treemap by City', height=450)
    
    state_sum = df.groupby('state', as_index=False)['sales'].sum().sort_values('sales', ascending=False)
    state_bar_fig = px.bar(state_sum.head(15), x='state', y='sales', 
                           title='Top 15 States by Sales', height=450)
    return city_treemap_fig, state_bar_fig


def create_store_type_fig(df):
    """Create store type sales timeline"""
    ...
    fig.update_layout(title="Sales Over Time by Store Type", xaxis_title="Date", 
                      yaxis_title="Sales", height=450)
    return fig
...
def create_monthly_trends_fig(df):
    ...
    monthly_trends_fig = px.line(monthly, x='month_name', y='sales', color='year', markers=True,
                                 ...
                                 title="Monthly Sales Trends by Year", height=450)
...
def create_time_series_metrics_fig(df):
    ...
    fig.update_layout(title="Time Series Metrics (Sales, Onpromotion, Oil Price)", 
...
def generate_dashboard():
    ...
    print("Merging data...")
    df = merge_auxiliary_data(train_raw, stores, holidays_cleaned, oil_cleaned)
    ...
    app.layout = html.Div([
        html.H1("Sales Analytics Dashboard", 
                style={'text-align': 'center', 'margin-bottom': '30px', 'color': '#2a3f5f'}),
        ...
        html.Div([
            ...
            html.Div([
                dcc.Graph(id='monthly-distribution', figure=monthly_box_fig, 
                         style={'width': '50%', 'display': 'inline-block', 'padding-right': '10px'}),
                dcc.Graph(id='weekday-analysis', figure=weekday_analysis_fig, 
        ...
        html.Div([
            ...
            html.Div([
                dcc.Graph(id='oil-scatter', figure=oil_scatter_fig, 
                         style={'width': '50%', 'display': 'inline-block', 'padding-right': '10px'}),
                dcc.Graph(id='oil-timeseries', figure=oil_timeseries_fig, 
                         style={'width': '50%', 'display': 'inline-block'})
            ], style={'display': 'flex'})
        ], style={'margin-bottom': '30px', 'border': '1px solid #ddd', 'padding': '15px'}),
        ...
        html.Div([
            ...
            html.Div([
                dcc.Graph(id='city-treemap', figure=city_treemap_fig, 
                         style={'width': '50%', 'display': 'inline-block', 'padding-right': '10px'}),
                dcc.Graph(id='state-bar', figure=state_bar_fig, 
                         style={'width': '50%', 'display': 'inline-block'})
            ], style={'display': 'flex'})
        ], style={'margin-bottom': '30px', 'border': '1px solid #ddd', 'padding': '15px'}),
        ...
        html.Div([
            ...
            html.Div([
                dcc.Graph(id='store-type-timeline', figure=store_type_fig, 
                         style={'width': '50%', 'display': 'inline-block', 'padding-right': '10px'}),
                dcc.Graph(id='monthly-trends', figure=monthly_trends_fig, 
                         style={'width': '50%', 'display': 'inline-block'})
            ], style={'display': 'flex'})
        ], style={'margin-bottom': '30px', 'border': '1px solid #ddd', 'padding': '15px'}),
        ...
        html.Div([
            ...
            dcc.Graph(id='time-series-metrics', figure=time_series_metrics_fig)
              ...
              'max-width': '1400px', 'margin': 'auto', 'background-color': '#f8f9fa'})
